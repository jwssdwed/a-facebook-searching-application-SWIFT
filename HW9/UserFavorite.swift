...
class UserFavorite: UIViewController, UITableViewDelegate, UITableViewDataSource {
    
    @IBOutlet weak var userFavTable: UITableView!
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        let cell:FbItems = tableView.dequeueReusableCell(withIdentifier: "userFavorite", for: indexPath)as! FbItems;
        cell.userFavName.text=names[indexPath.row]
        
        let url = NSURL(string: profiles[indexPath.row])
        if let data = NSData(contentsOf: url! as URL){
            cell.userFavProfile.image=UIImage(data: data as Data)
        }
        cell.userFavFavoriteButton.id = ids[indexPath.row]
        cell.userFavFavoriteButton.addTarget(self, action: #selector(SearchView.like(sender:)), for: .touchUpInside)
        return cell
    }
    ...
    func updateTable(){
        ...
        do{
            let results = try global.fetch(request)
            if results.count>0{
                print("let's print favorite sequence")
                for result in results as! [NSManagedObject]{
