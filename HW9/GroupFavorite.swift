...
import UIKit
import CoreData
class GroupFavorite: UIViewController,UITableViewDelegate,UITableViewDataSource {
    @IBOutlet weak var groupFavTable: UITableView!
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        let cell:FbItems = tableView.dequeueReusableCell(withIdentifier: "groupFavorite", for: indexPath)as! FbItems;
        cell.groupFavName.text=names[indexPath.row]
        
        let url = NSURL(string: profiles[indexPath.row])
        if let data = NSData(contentsOf: url! as URL){
            cell.groupFavProfile.image=UIImage(data: data as Data)
        }
        cell.groupFavFavoriteButton.id = ids[indexPath.row]
        cell.groupFavFavoriteButton.addTarget(self, action: #selector(SearchView.like(sender:)), for: .touchUpInside)
        return cell
    }
